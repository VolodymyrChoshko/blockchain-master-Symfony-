...
const BlockSection = () => {
  ...
  useEffect(() => {
    ...
    const section = el.classList.contains(constants.CLASS_BLOCK_SECTION);
  ...
  const handleUpdateClick = () => {
    setDisabled(true);
    activeEdits[0].classList[values.section ? 'add' : 'remove'](constants.CLASS_BLOCK_SECTION);
    ...
    Object.keys(zones).forEach((key) => {
      if (key.toString() === id.toString()) {
        zones[key].isSection = values.section;
      }
    });
  ...
  if (activeEdits.length === 0) {
    return (
      <div>
        <p className="mb-2">
          Enable sections as drag and drop stackable containers.
        </p>
        <img src="/assets/images/sections-graphic.png" alt="" style={{ width: '100%' }} />
      </div>
    );
  }
